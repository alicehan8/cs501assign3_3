...
fun ContactList(modifier: Modifier = Modifier){
    ...
    val scrollState = rememberLazyListState() //keep track of the scroll position
    val coroutineScope = rememberCoroutineScope() //allows for animate scroll to top

    //column for the entire screen
    Column(modifier = modifier){
        // row for the header -- title and button
        Row(modifier = Modifier.fillMaxWidth()
            .background(color = MaterialTheme.colorScheme.primaryContainer)
            .padding(vertical = 16.dp)) {
            //title for contacts screen
            Text(
                "Contacts",
            ...
            //display the fab to scroll to top when scrolled past 10 items
            if (scrollState.firstVisibleItemIndex > 10) {
        ...
        //lazy column for all entires in contacts list
        LazyColumn(modifier = Modifier.fillMaxSize(), state = scrollState){
            ContactsByLetter.forEach { letter ->
                //keep sticky header for each letter/group
                stickyHeader {
